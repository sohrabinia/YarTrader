...
def run_e2e_verification(auto_confirm: bool = False):
    ...
    trade_mode = sym_info.get("trade_mode", 0)
    # MT5 SYMBOL_TRADE_MODE constants: 0 = DISABLED, 4 = FULL
    session_data = {
        "symbol": "XAUUSD",
    ...
    save_artifact("06_session.json", session_data)

    if trade_mode == 0:  # SYMBOL_TRADE_MODE_DISABLED
        add_evidence("Market Session", "FAILED", f"XAUUSD trade_mode is DISABLED ({trade_mode}). Market is closed.")
        return print_final_verdict(evidence_table, "DEMO E2E BLOCKED — MARKET CLOSED", evidence_dir)
    ...
    # Check for Retcode 10018 (Market closed)
    if order_resp.Retcode == 10018:
        add_evidence("Real mt5.order_send()", "FAILED", "MT5 returned retcode 10018: Market closed")
        return print_final_verdict(evidence_table, "DEMO E2E BLOCKED — MARKET CLOSED", evidence_dir)
