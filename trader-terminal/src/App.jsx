...
function MainApp() {
  ...
  const [billingWallets, setBillingWallets] = useState([]);
  const [manualTxModalOpen, setManualTxModalOpen] = useState(false);
  const [txHashInput, setTxHashInput] = useState('');
  const [txPlanInput, setTxPlanInput] = useState('pro');
  const [txWalletInput, setTxWalletInput] = useState('');
  ...
  const fetchBillingWallets = async () => {
    try {
      const res = await apiService.get('/api/billing/wallets');
      if (res && res.verified_wallets) {
        setBillingWallets(res.verified_wallets);
      }
    } catch (err) {
      console.error('Failed to fetch billing wallets:', err);
    }
  };
  ...
  const handleManualTxSubmit = (e) => {
    e.preventDefault();
    if (!txHashInput.trim()) {
      showNotification(lang === 'fa' ? 'لطفاً هش تراکنش را وارد نمایید.' : 'Please enter transaction hash.', 'warning');
      return;
    }
    showNotification(
      lang === 'fa'
        ? 'تراکنش جهت بررسی دستی ثبت گردید. پس از تایید شبکه، اشتراک شما فعال خواهد شد.'
        : 'Transaction submitted for manual verification. Subscription will activate upon network confirmation.',
      'success'
    );
        ...
        <div className="main-panel">
          ...
          {hash === '#/pricing' && (
            <div id="shell-pricing">
              ...
              {/* VERIFIED PAYMENT WALLETS & NETWORK SAFETY SECTION */}
              <div className="card" style={{ marginTop: '25px', borderTop: '4px solid var(--primary)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
                  <h3 style={{ margin: 0, color: 'var(--primary)' }}>
                    💳 {lang === 'fa' ? 'کیف پول‌های تایید شده پرداخت اشتراک (Crypto Payment Wallets)' : 'Verified Subscription Payment Wallets'}
                  </h3>
                  <StatusBadge status="passed" label="PUBLIC RECEIVE ONLY" />
                </div>

                <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '20px', lineHeight: '1.6' }}>
                  {lang === 'fa'
                    ? 'جهت خرید یا ارتقای اشتراک، ارز موردنظر را فقط از طریق شبکه تایید شده به آدرس عمومی مربوطه واریز نمایید.'
                    : 'To subscribe or upgrade, transfer funds strictly on the indicated network to the verified public receive address below.'}
                </p>

                <div className="blog-grid" style={{ marginBottom: '20px' }}>
                  {billingWallets.map((w, wIdx) => (
                    <div key={wIdx} className="status-item" style={{ textAlign: 'inherit', padding: '15px', border: '1px solid var(--border-dark)', background: 'rgba(30, 41, 59, 0.2)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <span className="blog-tag" style={{ background: 'rgba(79, 70, 229, 0.2)', color: 'var(--primary)' }}>
                          {w.network}
                        </span>
                        <StatusBadge status={w.valid ? "passed" : "failed"} label={w.status} />
                      </div>
                      <div style={{ fontSize: '0.85rem', fontWeight: 'bold', color: 'var(--text-dark)', marginBottom: '6px' }}>
                        Asset: {w.asset}
                      </div>
                      <div style={{ fontFamily: 'monospace', fontSize: '0.78rem', background: 'rgba(0,0,0,0.3)', padding: '8px', borderRadius: '4px', wordBreak: 'break-all', marginBottom: '10px' }}>
                        {w.address}
                      </div>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                          ...
                          onClick={() => {
                            ...
                            showNotification(lang === 'fa' ? 'آدرس کیف پول کپی شد.' : 'Wallet address copied to clipboard.', 'success');
                          }}
                        >
                        ...
                        <button
                          ...
                          style={{ flex: 1, padding: '4px 8px', fontSize: '0.78rem' }}
                          onClick={() => {
                            setTxWalletInput(w.address);
                            setManualTxModalOpen(true);
                          }}
                        >
                          🔍 {lang === 'fa' ? 'ثبت هش واریز' : 'Submit Tx Hash'}
                        </button>
                      </div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--warning)', marginTop: '8px', lineHeight: '1.4' }}>
                        ⚠️ {w.warning}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ padding: '12px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--danger)', borderRadius: '6px', fontSize: '0.8rem', color: 'var(--danger)', lineHeight: '1.5' }}>
                  🛑 {lang === 'fa'
                    ? 'هشدار مهم شبکه: ارسال دارایی از طریق شبکه نادرست موجب از دست رفتن دائمی آن خواهد شد. همواره شبکه مبدا و مقصد را قبلاً تطبیق دهید.'
                    : 'Wrong-Network Warning: Transferring assets via an unsupported network will result in permanent loss. Always match networks before sending.'}
                </div>
              </div>

              {/* MANUAL TRANSACTION HASH SUBMISSION MODAL */}
              {manualTxModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.75)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                  <div className="card" style={{ width: '100%', maxWidth: '500px', background: 'var(--bg-dark)', border: '1px solid var(--primary)' }}>
                    <h3 style={{ marginTop: 0, color: 'var(--primary)' }}>🔍 {lang === 'fa' ? 'ثبت هش تراکنش جهت بررسی دستی' : 'Manual Transaction Verification'}</h3>
                    <form onSubmit={handleManualTxSubmit}>
                      <div className="form-group" style={{ marginBottom: '15px' }}>
                        <label className="form-label">{lang === 'fa' ? 'پلن انتخابی' : 'Target Subscription Plan'}</label>
                        <select className="select-field" style={{ width: '100%' }} value={txPlanInput} onChange={(e) => setTxPlanInput(e.target.value)}>
                          <option value="daily">Daily Pulse Plan ($29/mo)</option>
                          <option value="pro">Professional Analyst ($79/mo)</option>
                          <option value="institutional">Institutional SCM Terminal ($299/mo)</option>
                        </select>
                      </div>
                      <div className="form-group" style={{ marginBottom: '15px' }}>
                        <label className="form-label">{lang === 'fa' ? 'آدرس واریز شده' : 'Destination Receive Address'}</label>
                        <input type="text" className="input-field" value={txWalletInput} readOnly style={{ width: '100%', opacity: 0.8 }} />
                      </div>
                      <div className="form-group" style={{ marginBottom: '20px' }}>
                        <label className="form-label">{lang === 'fa' ? 'هش تراکنش (TxHash / Transaction Hash)' : 'Transaction Hash (TxHash)'}</label>
                        <input
                          type="text"
                          className="input-field"
                          placeholder="e.g. 0x1234... or txid..."
                          value={txHashInput}
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <button type="submit" className="btn" style={{ flex: 1 }}>
                          {lang === 'fa' ? 'تایید و ثبت جهت بررسی' : 'Submit Transaction'}
                        </button>
                        <button type="button" className="btn btn-secondary" style={{ flex: 1 }} onClick={() => setManualTxModalOpen(false)}>
                          {lang === 'fa' ? 'انصراف' : 'Cancel'}
                        </button>
                      </div>
                    </form>
