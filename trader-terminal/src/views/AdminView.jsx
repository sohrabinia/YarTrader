...
export default function AdminView({ t, devopsStatus, systemMetrics, usersList }) {
  ...
  const fetchOperatorStatus = async () => {
    try {
      ...
      const res = await fetch(`/api/admin/operator/status?token=${encodeURIComponent(token)}`);
      if (res.ok) {
  ...
  const handleTaskSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('yartrader_token') || '';
      const res = await fetch(`/api/admin/operator/tasks?token=${encodeURIComponent(token)}`, {
        method: 'POST',
