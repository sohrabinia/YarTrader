...
class PredictiveShadowEngine:
    ...
    def _record_pattern_outcome_context(self, ctx: SymbolTimeContext, trade: ShadowTrade) -> None:
        ...
        # Sanitize trade_id to prevent any potential path traversal vulnerability
        safe_trade_id = "".join(c for c in trade.trade_id if c.isalnum() or c in "-_")
        snapshot_filepath = os.path.join(brain_memory_dir, f"pattern_{safe_trade_id}.json")
        try:
            with open(snapshot_filepath, "w", encoding="utf-8") as f:
                json.dump(outcome, f, indent=4)
        except Exception as e:
            logger.error("Failed to write snapshot to brain_memory: %s", e)
