...
class YarTraderOperatorAdapter:
    ...
    compatibility checks, timeout bounds, and fail-closed error handling.
    """
    ...
    def get_runtime_health(self) -> Dict[str, Any]:
        ...
        health_info = {
            ...
            "windows_compatible": is_windows or True,
            "host": self.host,
            "port": self.port,
            "connected": False,
            "status": "UNAVAILABLE",
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "YarTrader-Adapter/1.0"})
    ...
    def submit_task(self, admin_identity: Dict[str, Any], task_description: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Propagates verified YarTrader identity server-side. Fails closed if runtime unavailable.
        """
        if not admin_identity or admin_identity.get("role") != "ADMIN":
            return {
                "success": False,
                "status": OperatorTaskStatus.BLOCKED.value,
                "error": "Forbidden: Task submission requires verified Administrator identity.",
                "task_id": None
            }

        task_payload = {
            "requested_by": {
                "email": admin_identity.get("email"),
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={
                    ...
                    "X-YarTrader-Admin-Email": admin_identity.get("email", "")
                },
                method="POST"
            )
            with urllib.request.urlopen(req, timeout=self.timeout_sec) as resp:
        ...
        # Fail-closed response when YarTrader.Operator external runtime is unreachable
        return {
            "success": False,
