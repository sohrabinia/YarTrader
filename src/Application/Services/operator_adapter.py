...
class YarTraderOperatorAdapter:
    ...
    def submit_task(self, admin_identity: Dict[str, Any], task_description: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        authenticated_via = admin_identity.get("authenticated_via") or "YarTrader_Session"
    ...
    def get_all_tasks(self, admin_identity: Dict[str, Any]) -> Dict[str, Any]:
        """
        Retrieves list of all active/historical tasks from YarTrader.Operator.
        """
        if not admin_identity or admin_identity.get("role") != "ADMIN" or not admin_identity.get("email"):
            return {
                "success": False,
        ...
        try:
            ...
            logger.error(f"Failed to fetch task history: {str(e)}")

        return {
            "success": False,
            "error": f"Unable to fetch task list from YarTrader.Operator ({self.base_url}).",
            "tasks": []
        }
