...
class IdentityAuthority:
    ...
    def issue_identity_assertion(
        ...
        Fails closed on missing user_id, owner_id, workspace_id, or inaccessible CNG key container without fallback.
        """
        if not user_id or not isinstance(user_id, str):
            raise ValidationException("Identity assertion requires a non-empty user_id.")

        if not owner_id or not isinstance(owner_id, str):
            raise ValidationException("Identity assertion requires an explicit non-empty owner_id without fallback.")

        if not workspace_id or not isinstance(workspace_id, str):
            raise ValidationException("Identity assertion requires an explicit non-empty workspace_id without fallback.")
