...
class DailyLossKillSwitch:
    ...
    def evaluate_daily_loss(
        ...
        try:
            ...
            # If no baseline established at all yet, attempt initialization from session_baseline_equity or current_equity
            if self.baseline_equity is None or not math.isfinite(self.baseline_equity) or self.baseline_equity <= 0:
                init_candidate = session_baseline_equity if session_baseline_equity is not None else current_equity
                if isinstance(init_candidate, (int, float)) and not isinstance(init_candidate, bool):
                    b_val = float(init_candidate)
