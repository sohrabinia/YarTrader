...
1. **URL Preservation:** All existing valid public, authenticated, blog, guide, FAQ, and admin URLs across all 4 production locales are strictly frozen. No URL may be deleted or modified without a 301 permanent redirect.
2. **Dynamic Locale Prefix Routing:** Every public route is accessible both un-prefixed (defaulting to Persian `fa` or user preference) and explicitly prefixed with `/{lang}/` where `lang` is in `['fa', 'en', 'tr', 'ar']`.
3. **No Soft 404s:** Unknown public subpaths return a strict HTTP 404 response in API contexts or structured localized fallback in SPA routing.
4. **Static SEO Endpoints:**
...
| `/{lang}/pricing` | SaaS Plans & Prop Challenge Gate | Public | `https://yartrader.com/{lang}/pricing` | `INDEX, FOLLOW` |
...
| `/{lang}/shadow` | Paper Execution Engine | Authenticated | `https://yartrader.com/{lang}/shadow` | `NOINDEX, NOFOLLOW` |
