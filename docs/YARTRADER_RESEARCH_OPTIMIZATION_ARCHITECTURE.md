...
**Repository Version / Commit SHA:** `ae3f65d79b34c8f46a43ee783a41b1841fb76364`
...
PARAMETER SPACE GRID SEARCH (CARTERSIAN PRODUCT)
          │
          ▼
RESEARCH BACKTEST SIMULATOR (WITH PARAMETER INJECTION & COSTS)
          │
          ▼
...
## 3. CORE COMPONENTS & PARAMETER INJECTION

### 3.1 Research Parameter Injection (`src/Application/Backtesting/backtest_learning_engine.py`)
`BacktestAndLearningEngine.run_backtest()` accepts an optional `strategy_params: Optional[Dict[str, Any]] = None` dictionary. When provided during research optimization:
- `confidence_threshold`: Overrides minimum confidence required to accept trade setups.
- `minimum_rr`: Overrides minimum risk-reward gate required to trigger trade setups.
- `stop_multiplier`: Scales stop-loss distances for candidate stop evaluation.
- `target_multiplier`: Scales target take-profit multipliers for candidate reward evaluation.

When `strategy_params` is None (the default), `run_backtest()` defaults 100% to frozen core behavior.

### 3.2 Parameter Space Abstraction (`src/Application/Research/optimization/parameter_space.py`)
...
### 3.3 Dataset Splitter (`src/Application/Research/optimization/dataset_splitter.py`)
...
### 3.4 Cost-Adjusted Cost Model (`src/Application/Research/optimization/cost_model.py`)
...
### 3.5 Multi-Objective Evaluator (`src/Application/Research/optimization/objective.py`)
...
### 3.6 Grid Search Engine (`src/Application/Research/optimization/grid_search.py`)
Executes exhaustive parameter sweeps against backtest simulations with built-in failure isolation and passes candidate parameters to `run_backtest(strategy_params=config)`.

### 3.7 Walk-Forward Optimizer (`src/Application/Research/optimization/walk_forward.py`)
...
### 3.8 Overfitting Diagnostics (`src/Application/Research/optimization/overfitting.py`)
...
### 3.9 Baseline Evaluator (`src/Application/Research/optimization/baseline.py`)
...
### 3.10 Experiment Provenance & Reporting (`src/Application/Research/optimization/provenance.py` & `report.py`)
