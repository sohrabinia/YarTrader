...
3. `JULES_TEST`: Temporary test fixtures or synthetic data created during sandbox testing.
4. `UNKNOWN`: Unverified data records subject to safety quarantine.
...
| **Subscription Plans** | `SYSTEM_REQUIRED` | Yes | 4 canonical tiers (`free`, `daily`, `pro`, `institutional`) defined in `content_manager.py`. |
| **Version Metadata** | `SYSTEM_REQUIRED` | Yes | Dynamic version resolution (`YarTrader v7.0`). |
| **UI Mock Fallbacks** | `JULES_TEST` | **REMOVED** | Removed fake data generators. Unavailable APIs now return clean `DATA UNAVAILABLE` states. |
| **Demo MT5 Orders** | `REAL_YARTRADER` | Yes (Demo Mode) | Real order placement on Alpari MT5 Demo account #52961173. |
| **Shadow Paper Account** | `REAL_YARTRADER` | Yes (Shadow Mode) | Virtual $1,000 account simulated in `ShadowTradingEngine`. |

---

## 3. Production Seed Protection & Safety Verification

1. **Zero Fake Fallback Rule:** Frontend components in `App.jsx`, `DashboardView.jsx`, `IntelligenceView.jsx`, and `AdminView.jsx` do NOT fall back to fake numbers or synthetic trades when backend endpoints return empty or offline status. Empty states (`EmptyState.jsx`) or `DATA UNAVAILABLE` badges are displayed.
