...
SERVICE_LOG_DIR = os.path.join(YarTraderStorageManager.get_manager().get_log_dir(), "service")
...
def log_service_message(message: str) -> None:
    ...
    try:
        with open(os.path.join(SERVICE_LOG_DIR, "service.log"), "a", encoding="utf-8") as f:
            f.write(log_entry)
    except Exception:
        pass
    print(message)
