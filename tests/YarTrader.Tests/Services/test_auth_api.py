...
class TestSaaSAuthAPI(unittest.TestCase):
    ...
    def test_customer_email_password_registration_and_login_policy_enforcement(self) -> None:
        """Verifies that non-admin customer email/password registration and login are rejected per Google-Only policy."""
        import uuid
        test_email = f"newtrader-{uuid.uuid4().hex[:6]}@yartrader.app"
        test_password = "SecurePassword123!"

        # 1. Customer Registration Rejection
        ...
        self.assertIn("Customer registration requires Google Sign-In", reg_resp.json()["detail"])

        # 2. Customer Login Rejection
        login_resp = self.client.post("/api/auth/login", json={
            "email": test_email,
            "password": test_password
        })
        self.assertEqual(login_resp.status_code, 400)
        self.assertIn("Customer authentication requires Google Sign-In", login_resp.json()["detail"])
    ...
    def test_registration_security_boundary_rejection_for_existing_accounts(self) -> None:
        ...
        self.assertIn("Customer registration requires Google Sign-In", reg_google_resp.json()["detail"])
    ...
    def test_authenticated_password_recovery_flow_preserves_identity_and_data(self) -> None:
        ...
        # 6. Verify customer password login is rejected per Google-Only policy
        login_resp = self.client.post("/api/auth/login", json={
            "email": test_email,
    ...
    def test_admin_role_granted_via_email_login_and_operator_authorization(self) -> None:
        ...
        user_social = global_auth_service.authenticate_social(email=user_reg_email, provider="google", provider_id="sub-normal")
        user_token = global_auth_service.create_session(user_social)
    ...
    def test_admin_password_login_and_session_verification(self) -> None:
        ...
        with patch.dict(os.environ, {"YARTRADER_DEFAULT_ADMIN_PASSWORD_HASH": admin_hash}):
            ...
            # 2. Non-Admin Email Rejection per Google-Only policy (Scenario E)
            unknown_email_resp = self.client.post("/api/auth/login", json={
                "email": "nonexistent-admin@yartrader.app",
                "password": admin_password
            })
            self.assertEqual(unknown_email_resp.status_code, 400)
            self.assertIn("Customer authentication requires Google Sign-In", unknown_email_resp.json()["detail"])
