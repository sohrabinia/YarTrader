...
def test_mt4_adapter_has_zero_production_execution_authority():
    """Proves MT4 adapter raises ValidationException on order submission and rejects production orders."""
    mt4 = RealMT4BrokerAdapter(auto_initialize=False)

    acc = mt4.get_account_info()
    assert acc is not None
    assert acc["platform"] == "MT4"
    assert acc["is_real_broker"] is False
