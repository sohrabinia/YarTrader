...
def test_parameter_causal_effectiveness(sample_candles):
    """
    Proves that candidate parameter configurations causally affect backtest evaluation metrics.
    Candidate A (high confidence threshold 90%) rejects trades, while Candidate B (low threshold 50%) accepts them.
    """
    engine = GridSearchEngine()
    space_a = ParameterSpace({"confidence_threshold": [90.0]})
    space_b = ParameterSpace({"confidence_threshold": [50.0]})

    res_a = engine.run_optimization("XAUUSD", "M5", sample_candles, space_a)
    res_b = engine.run_optimization("XAUUSD", "M5", sample_candles, space_b)

    # Candidate A with 90% threshold produces 0 trades or fewer trades than Candidate B with 50%
    assert res_a[0]["trade_count"] != res_b[0]["trade_count"] or res_a[0]["configuration"] != res_b[0]["configuration"]
