...
def test_portfolio_risk_evaluator_safety():
    ...
    assert evaluator.check_allocation_safety(weights, params) is True

    # Check exposure violation (exceeds single asset exposure max of 20%)
    unsafe_exposure_weights = {"AAPL": 0.25, "MSFT": 0.05}
    assert evaluator.check_allocation_safety(unsafe_exposure_weights, params) is False

    # Check leverage violation (total weight = 1.2 > 1.0)
    unsafe_leverage_weights = {"AAPL": 0.20, "MSFT": 1.0}
    assert evaluator.check_allocation_safety(unsafe_leverage_weights, params) is False
